...
my_data_base = myclient[sys.argv[2]]
...
Team MESA Library.'''
    s.sendmail(sys.argv[3], email, message)
...
def new_book_entry():
    ...
    for l in range(0, n):
        ...
        avail = 'Available'
        add_new_book = {'Book id': book_id, 'Book Name': book, 'Doner Name': doner, 'Doner Email': doner_mail,
                        'Author Name': author, 'Book Category': category, 'Date (YYYY/MM/DD)': date,
                        'Time (HH:MM:SS)': time, 'Availability Status': avail}
...
def issue_book():
    ...
    for m in range(0, n):
        ...
        author = input('Enter author name:')
        category = input('Enter book category:')
        c = change_availibility_status(
            {'Book Name': re.compile(book, re.IGNORECASE), 'Author Name': re.compile(author, re.IGNORECASE),
             'Book Category': re.compile(category, re.IGNORECASE), 'Availability Status': 'Available'})
        if c != 0:
            ...
            add_new_book = {'Book id': c, 'Book Name': book, 'Issuer Name': issuer, 'Issuer Email': issuer_mail,
                            'Author Name': author, 'Book Category': category, 'Date (YYYY/MM/DD)': date,
                            'Time (HH:MM:SS)': time, 'Deadline (YYYY/MM/DD)': deadline, 'Fine': fine, 'Pay Status': pay,
...
def check_fine():
    ...
    for s in searched:
        deadline = datetime.datetime.fromtimestamp(
            datetime.datetime.fromisoformat(s['Deadline (YYYY/MM/DD)']).timestamp()).date()
...
def return_book():
    ...
    if (fine_check['Fine'] > 0):
        deadline = datetime.datetime.fromtimestamp(
            datetime.datetime.fromisoformat(fine_check['Deadline (YYYY/MM/DD)']).timestamp()).date()
        today = datetime.datetime.now().date()
        day = (today - deadline).days
        ...
        status = status.lower()
        if (status == 'yes'):
            database_object.find_one_and_update(fine_check, {
                "$set": {'Pay Status': 'Paid', 'Fine': 0, 'Return Status': 'Returned'}})
            database_object1.find_one_and_update({'Book id': re.compile(book_id, re.IGNORECASE)},
...
def search_by_issuer_name():
    ...
    print(end='\n')
    query = {'Issuer Name': re.compile(issuer_name, re.IGNORECASE), 'Pay Status': re.compile(pay_status, re.IGNORECASE),
             'Return Status': re.compile(return_status, re.IGNORECASE)}
    searched = database_object.find(query)
    for i in searched:
...
def search_by_book_name():
    ...
    return_status = input('Enter return status (Returned or Not Returned):-')
    print(end='\n')
    query = {'Book Name': re.compile(book_name, re.IGNORECASE), 'Pay Status': re.compile(pay_status, re.IGNORECASE),
             'Return Status': re.compile(return_status, re.IGNORECASE)}
    searched = database_object.find(query)
    for j in searched:
...
def search_by_date():
    ...
    return_status = input('Enter return status (Returned or Not Returned):-')
    print(end='\n')
    query = {'Date (YYYY/MM/DD)': issue_date, 'Pay Status': re.compile(pay_status, re.IGNORECASE),
             'Return Status': re.compile(return_status, re.IGNORECASE)}
    searched = database_object.find(query)
    for k in searched:
...
def search_by_deadline():
    ...
    return_status = input('Enter return status (Returned or Not Returned):-')
    print(end='\n')
    query = {'Deadline (YYYY/MM/DD)': deadline_date, 'Pay Status': re.compile(pay_status, re.IGNORECASE),
             'Return Status': re.compile(return_status, re.IGNORECASE)}
    searched = database_object.find(query)
    for q in searched:
...
def search_by_book_author():
    ...
    return_status = input('Enter return status (Returned or Not Returned):-')
    print(end='\n')
    query = {'Author Name': re.compile(author_name, re.IGNORECASE), 'Pay Status': re.compile(pay_status, re.IGNORECASE),
             'Return Status': re.compile(return_status, re.IGNORECASE)}
    searched = database_object.find(query)
    for r in searched:
...
def search_by_book_category():
    ...
    return_status = input('Enter return status (Returned or Not Returned):-')
    print(end='\n')
    query = {'Book Category': re.compile(book_category, re.IGNORECASE),
             'Pay Status': re.compile(pay_status, re.IGNORECASE),
             'Return Status': re.compile(return_status, re.IGNORECASE)}
    searched = database_object.find(query)
    for s in searched:
...
def search_by_book_id():
    ...
    return_status = input('Enter return status (Returned or Not Returned):-')
    print(end='\n')
    query = {'Book id': re.compile(book_id, re.IGNORECASE), 'Pay Status': re.compile(pay_status, re.IGNORECASE),
             'Return Status': re.compile(return_status, re.IGNORECASE)}
    searched = database_object.find(query)
    for v in searched:
